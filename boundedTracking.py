    ...
    while not _stop_event.is_set():
        ...
        if ids is not None:
            ...
            for i, idv in enumerate(ids.flatten()):
                if idv in marker_positions:
                    cam_t = tvecs[i][0]
                    pos_x = marker_positions[idv][0] - cam_t[0]
                    pos_y = marker_positions[idv][1] - cam_t[1]
        # YOLO inference (silenced)
        results = model(img, imgsz=480, verbose=False)
        ...
        cy = ((x1+x2)/2/640 - 0.5)
        int_y += cy*dt; der_y=(cy-prev_y)/dt
        yaw = int(np.clip(KP_Y*cy+KI_Y*int_y+KD_Y*der_y, -100,100))
        ef = WIDTH_SET - ((x2-x1)/640)
        int_f += ef*dt; der_f=(ef-prev_f)/dt
        fb = int(np.clip(KP_F*ef+KI_F*int_f+KD_F*der_f, -100,100))
        eu = TOP_SET - (y1/480)
        int_u += eu*dt; der_u=(eu-prev_u)/dt
...
def video_thread():
    ...
    while len(marker_positions)<args.num_markers and time.time()-start<args.scan_timeout:
        ...
        if ids is not None:
            rvecs,tvecs,_=aruco.estimatePoseSingleMarkers(corners,args.marker_size,cam_mtx,dist_coefs)
            for i,idv in enumerate(ids.flatten()):
                if idv not in marker_positions: marker_positions[idv]=tvecs[i][0]; print(f"[SCAN] #{idv}")
    send_rc(0,0,0,0); print("[SCAN] markers done")
    ...
    while not found and time.time()-start<args.scan_timeout:
        frame=get_latest_frame();
        if frame is None: continue
        res=model(cv2.resize(frame,(640,480)),imgsz=480,verbose=False)
        if any(c==0 for c in res[0].boxes.cls.cpu().numpy().astype(int)): found=True
    send_rc(0,0,0,0); print("[SCAN] person done")
