...
parser.add_argument("--scan-timeout", type=float, default=20.0, help="Timeout (s) for scanning loops")
parser.add_argument("--pid-delay", type=float, default=0.05, help="Delay (s) between PID loop iterations")
...
# 5) Keep-alive thread to prevent auto-landing
def keepalive_thread():
    while not _stop_event.is_set():
        send_command("command")   # resets timeout
        time.sleep(5)

# 6) Calibration & ArUco setup
...
def inference_thread(model):
    ...
    while not _stop_event.is_set():
        ...
        if frame is None:
            time.sleep(args.pid_delay); continue
        img = cv2.resize(frame, (640, 480))
        res = model(img, imgsz=480, verbose=False)
        annotated = res[0].plot(); cv2.imshow("Tello YOLOv8", annotated)
        ...
        if not persons:
            time.sleep(args.pid_delay); continue
        # PID calculations
        x1, y1, x2, y2 = persons[int(np.argmax([(b[2]-b[0])*(b[3]-b[1]) for b in persons]))]
        t = time.time(); dt = max(t - prev_t, 1e-6)
        ...
        time.sleep(args.pid_delay)
    cv2.destroyAllWindows()

# 9) Video & scanning thread
def video_thread():
    model = YOLO("yolov8n.pt")
    print("[VIDEO] waiting for stream dir...")
    ...
    if not cap.isOpened(): print("[VIDEO] failed"); _stop_event.set(); return
    print("[VIDEO] opened")
    threading.Thread(target=capture_thread, args=(cap,), daemon=True).start()

    cv2.namedWindow("Tello Markers", cv2.WINDOW_NORMAL)
    print(f"[SCAN] markers→{args.num_markers}")
    send_rc(0, 0, 0, args.scan_yaw_speed)
    start = time.time()
    while len(marker_positions) < args.num_markers and time.time()-start < args.scan_timeout:
        frame = get_latest_frame();
        if frame is None: continue
        cv2.imshow("Tello Markers", frame); cv2.waitKey(1)
        ...
        if ids is not None:
            ...
            for i, idv in enumerate(ids.flatten()):
                if idv not in marker_positions:
                    cam_t = tvecs[i][0]
                    marker_positions[idv] = (cam_t[0], cam_t[2])
                    print(f"[SCAN] #{idv}")
    # keepalive prevents auto-land instead of zeroing RC
    print("[SCAN] markers done")

    xs = [v[0] for v in marker_positions.values()]
    zs = [v[1] for v in marker_positions.values()]
    inset = 0.3048
    global boundary
    boundary = (min(xs)+inset, max(xs)-inset, min(zs)+inset, max(zs)-inset)
    print(f"[BOUNDARY] x[{boundary[0]:.2f},{boundary[1]:.2f}] z[{boundary[2]:.2f},{boundary[3]:.2f}]")

    # Fly boundary path
    for x_m, z_m in [(boundary[0],boundary[2]),(boundary[0],boundary[3]),(boundary[1],boundary[3]),(boundary[1],boundary[2])]:
        send_command(f"go {int(x_m*100)} {int(z_m*100)} 0 20"); time.sleep(5)
    cx = int(((boundary[0]+boundary[1])/2)*100)
    cz = int(((boundary[2]+boundary[3])/2)*100)
    send_command(f"go {cx} {cz} 0 20"); time.sleep(5)

    print("[SCAN] locating person")
    send_rc(0, 0, 0, args.scan_yaw_speed)
    start = time.time(); found=False
    while not found and time.time()-start < args.scan_timeout:
        frame = get_latest_frame();
        if frame is None: continue
        if any(c==0 for c in YOLO("yolov8n.pt")(cv2.resize(frame,(640,480)),imgsz=480,verbose=False)[0].boxes.cls.cpu().numpy().astype(int)):
            found=True
    send_rc(0, 0, 0, 0)
    print("[SCAN] person found, starting tracking")
    threading.Thread(target=inference_thread,args=(model,),daemon=True).start()

# 10) Main entry
if __name__ == "__main__":
    send_command("command"); send_command("streamon"); send_command("takeoff");
    # start keepalive to prevent auto-landing
